...
`simple_python()`. Authentication is via a token, which is typically
shared across CAVE projects (e.g. flywire, FANC, BANC etc). You can make
a new one linked to the email address authorised to access web resources
via neuroglancer.
...
interface but you can get one by calling `flytable_set_token()` with
your flytable user and password. This should be a once only step.
